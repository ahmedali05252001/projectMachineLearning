...
\author{
  Solo coursework project (single author)\\
  \texttt{replace.with@student.email} \quad GitHub: \texttt{ahmedali05252001/projectMachineLearning}
}
...
After constructing the strictly pre-match modeling table (rolling history shifted by one match per team timeline, plus normalized implied probabilities from Bet365 closing odds), we retain \(n{=}7079\) labeled matches spanning five European divisions across four downloaded seasons. The full-time label distribution is mildly imbalanced: home wins occur in about \(43.4\%\) of rows, away wins in \(31.3\%\), and draws in \(25.3\%\). This ordering is consistent with home advantage in football and motivates reporting macro-F1 alongside accuracy. The notebook accompanying this repository provides league-wise counts, bivariate views of implied probabilities versus outcomes, and correlation checks among engineered numeric features.

\section{Results}
Table~\ref{tab:cv} reports five-fold stratified cross-validation means with standard deviations across folds for accuracy, macro-F1, weighted-F1, and multiclass log loss. All models use the same folds and preprocessing pipeline (standardized numeric features; one-hot encoded division). The table body is generated automatically by \texttt{tools/export\_cv\_results.py} to keep the manuscript synchronized with the repository evaluation code.

\begin{table}[t]
\centering
\caption{Five-fold stratified CV performance (mean \(\pm\) std). Log loss is the multiclass cross-entropy computed from predicted probabilities where available.}
\label{tab:cv}
\input{generated_cv_table.tex}
\end{table}

\noindent\textbf{Reading the table.} Logistic regression achieves the strongest weighted-F1 in this configuration, while the RBF SVM attains the best (lowest) log loss by a small margin. XGBoost is competitive on accuracy but exhibits lower macro-F1, suggesting comparatively weaker per-class balance on the draw class under the chosen hyperparameters---an observation corroborated by confusion matrices in the notebook. These patterns are consistent with a setting where market-implied probabilities already encode much of the predictable variation, leaving limited headroom for nonlinear enrichment without careful tuning or additional signal.
