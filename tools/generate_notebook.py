...
nb_path = ROOT / "notebooks" / "soccer_match_hda_classification.ipynb"
...
cells = [
    md(
        ...
        "This notebook builds a **strictly pre-match** modeling table for **multi-class** outcomes "
        "(`H`=home win, `D`=draw, `A`=away win) using:\n\n"
    ...
    md("## 2) EDA (dataset + target + key pre-match signals)"),
    code(
        "model_df.shape"
    ),
    code(
    ...
    md(
        "## 3) Modeling setup\n\n"
        "We use the **same** `StratifiedKFold(5)` for every model and report **accuracy**, "
        "**macro/weighted F1**, and **negative log loss** (multiclass)."
    ),
    code(
        "NUMERIC_FEATURES = [\n"
        "    \"imp_h\",\n"
    ...
    md("## 4) Confusion matrices (out-of-fold predictions, same CV)"),
    code(
        "labels = [\"H\", \"D\", \"A\"]\n\n"
    ...
    md("## 5) Example classification report (best CV accuracy model)"),
    code(
    ...
    md(
        ...
        "- **Odds** are closing prices; they encode public information — still legitimate **pre-kickoff** inputs, "
        "but discuss **market efficiency** and **ethical/betting** context.\n"
        "- **Alternative validation**: chronological / season-based splits often better reflect deployment; "
        "we used stratified CV here for stable multi-class metrics across models.\n"
    ),
]
